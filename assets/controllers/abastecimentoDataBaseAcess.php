    ...
    foreach($lista as $row){
        ...
        if($row['combustivel'] <> 'GASOLINA'){
            ...
            if($row['erro_status'] == 1){
                $anularErro = '<td id="'.$row['id_abastecimento'].'" onclick="alterarStatusErro('.$row['id'].','.$row['id_abastecimento'].')" class="w3-green" style="cursor:pointer"><center>NÃO</td>';
            }else{
                $anularErro = '<td id="'.$row['id_abastecimento'].'" onclick="alterarStatusErro('.$row['id'].','.$row['id_abastecimento'].' )" class="w3-red" style="cursor:pointer"><center>SIM</td>';
            }
